...
- `research.html` and `src/research.js` render the investment research workspace; `src/research-ui.js` renders each of its tabs and wires the controls that recalculate the engines.
- `src/core/research/` is the research foundation, built once and shared by every research feature. `dataset.js` holds the reference securities, fundamentals, earnings, dividends, price history, and macro series, and `provenance.js` wraps every externally sourced value with its provider, source, as-of timestamp, and currency, or records it as explicitly missing. `stats.js` supplies the shared statistics.
- The analytics engines are pure functions over that foundation: `screener.js`, `dcf.js`, `risk.js`, `technicals.js`, `earnings.js`, `portfolio-builder.js`, `dividends.js`, `competition.js`, `quant.js`, and `macro.js`. `workspace.js`, `watchlists.js`, `portfolios.js`, and `reports.js` provide global search, the company header, watchlists and alerts, research portfolios, and immutable report snapshots that keep a saved report's assumptions and data as of the moment it was generated.
- `orchestrator.js` assembles a grounding packet — security metadata, market data, financial statements, calculated metrics, macro data, and user assumptions — and interprets it. Interpretation only narrates values it was given: it never calculates the financial models itself and lists fields the connected sources do not provide instead of estimating them.
